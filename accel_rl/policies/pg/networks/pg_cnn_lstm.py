...
import accel_rl.policies.layers as AL


class PgCnnLstm(BaseNetwork):
    """
    ...
    def __init__(
            ...
            hidden_W_init=AL.NormCInit(1.0),
            output_pi_W_init=AL.NormCInit(0.01),  # (like in OpenAI baselines)
            output_v_W_init=AL.NormCInit(1.0),
            all_b_init=LI.Constant(0.),
            hidden_h_init=LI.Constant(0.),
            cell_c_init=LI.Constant(0.),
        ...
        x = AL.ScalarFixedScaleLayer(l_in, scale=pixel_scale, name='pixel_scale')
        ls_conv = list()
        ...
        for i, hidden_size in enumerate(hidden_sizes):
            ...
            l_prev_cell = L.InputLayer(
                shape=(None, hidden_size),
                name="%scprev_%d_input" % (prefix, i),
            )
            x = AL.FastLstmLayer(
                incomings=(x, l_prev_hidden, l_prev_cell),
                num_units=hidden_size,
                nonlinearity=hidden_nonlinearity,
                name="%shidden_lstm_%d" % (prefix, i),
                W=hidden_W_init,
                b=all_b_init,
                h0=hidden_h_init,
                c0=cell_c_init,
            ...
            ls_prev_hidden.extend([l_prev_hidden, l_prev_cell]) # For the lstm the hidden state is (h, c)
            ls_recurrent.append(x)
            x = AL.SelectOutputLayer(x, 0)
    ...
    def hid_init_params(self):
        return sum([[layer.h0, layer.c0] for layer in self._ls_recurrent],[])
