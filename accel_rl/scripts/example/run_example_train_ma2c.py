...
script = "accel_rl/scripts/example/example_train_ma2c.py"

# n_gpu, ctx_per_gpu, ctx_per_run, n_cpu_cores, n_sockets, (ht_offset)
affinity_code = "4gpu_1cxg_4cxr_20cpu_2skt"  # (write according to computer specs)
...
games = ["alien", "beam_rider", "breakout", "space_invaders", "zaxxon"]
...
learning_rates = [2.5e-4, 2.5e-4, 2.5e-4]
n_envs = [64, 128, 256]

exps = list(zip(learning_rates, n_envs))

sub_names = ["ma2c_{}lr_{}envs".format(*exp) for exp in exps]
...
run_GPU_exps(script, affinity_code, args_list)
